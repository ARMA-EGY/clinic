            ...
            <div class="col-lg-12 text-left">
              <nav aria-label="breadcrumb" class="d-none d-md-inline-block ml-md-4">
                <ol class="breadcrumb breadcrumb-links breadcrumb-dark">
                  ...
                  <li class="breadcrumb-item"><a href="{{route('staff-external-appointment.index')}}">{{__('master.EXTERNAL-APPOINTMENTS')}}</a></li>
...
@section('script')
    ...
    <script>
        ...
        $(document).on("click",".choose-card", function()
        {
            ...
            $.ajax({
                    url: "{{route('staff-external-appointment.next')}}",
                    type:"POST",
                    dataType: 'text',
                    data:    all_objects,
        ...
        $(document).on("click",".prev-step", function()
        {
            ...
            $.ajax({
                    url: "{{route('staff-external-appointment.prev')}}",
                    type:"POST",
                    dataType: 'text',
                    data:    all_objects,
                    success : function(response)
        ...
        $(document).on('submit', '.appointment_form', function(e)
        {
            ...
            $.ajax({
                url: 		"{{route('staff-external-appointment.store')}}",
                method: 	'POST',
                data: formData,
                dataType: 	'json',
                contentType: false,
                ...
                success : function(data)
                    {
                        ...
                        if (data['status'] == 'true')
                        {
                            ...
                            {
                                window.location.href = "{{route('staff-external-appointment.index')}}";
                            }, 1000);
                        }
