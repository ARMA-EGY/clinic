            ...
            <div class="col-lg-12 col-12 text-left">
              <nav aria-label="breadcrumb" class="d-none d-md-inline-block ml-md-4">
                <ol class="breadcrumb breadcrumb-links breadcrumb-dark">
                  ...
                  <li class="breadcrumb-item active" aria-current="page">{{__('master.PATIENTS')}}</li>
                </ol>
              </nav>
            </div>
          ...
          <div class="card">
            <div class="card-header border-0">
              <div class="row align-items-center">
                <div class="col">
                  <h3 class="mb-0">{{__('master.PATIENTS')}} <span class="badge badge-primary p-2">{{$total_rows}}</span></h3>
                </div>
              </div>
            </div>
            ...
            <div class="table-responsive">
              ...
              <table class="table align-items-center table-flush display nowrap" id="example">
                <thead class="thead-light">
                  <tr>
                    <th scope="col">#</th>
                    <th scope="col" class="sort" >{{__('master.REGISTERED-DATE')}}</th>
                ...
                <tbody>
                  ...
                  <tr class="parent">
                    ...
                    <td><strong>{{ $item->identifiation }}</strong></td>
                    <td><strong> {{  $item->name }}</strong> </b></td>
                    <td><strong>{{ $item->phone }}</strong></td>
                    <td><strong>{{ $item->dateofbirth }} </strong> </td>
                    <td><strong>{{ $item->age }} </strong> </td>
                    <td><strong>{{__('master.'.$item->gender )}}</strong> </td>
                    <td><strong>{{__('nationality.'.$item->nationality )}} </strong></td>
                  </tr>
                ...
                <tfoot>
                    <tr>
                        ...
                        <th scope="col" class="sort p-2" >{{__('master.REGISTERED-DATE')}}</th>
                        <th scope="col" class="sort p-2" >{{__('master.IDENTIFICATION')}}</th>
                        <th scope="col" class="sort p-2" >{{__('master.NAME')}}</th>
                        <th scope="col" class="sort p-2" >{{__('master.PHONE')}}</th>
                        <th scope="col" class="sort p-2" >{{__('master.BIRTHDATE')}} </th>
                        <th scope="col" class="sort p-2" >{{__('master.AGE')}}</th>
                        <th scope="col" class="sort p-2" >{{__('master.GENDER')}}</th>
                        <th scope="col" class="sort p-2" >{{__('master.NATIONALITY')}}</th>  
                    </tr>
                </tfoot>
              </table>
            </div>
      ...
      $('#example tfoot th').each( function () {
         ...
         var table = $('#example').DataTable({
            ...
            buttons: [
                        ...
                        { extend: 'excel', title: 'Patients Report', className: 'btn btn-sm btn-success' },
                        { extend: 'print', text: '{{__("master.PRINT")}}', title: 'Patients Report', className: 'btn btn-sm btn-primary' },
                    ]
         });
